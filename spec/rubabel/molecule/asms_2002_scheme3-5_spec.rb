...
describe Rubabel::Molecule::Fragmentable do
  ...
  describe 'JASMS 2002 scheme 3' do 
    specify 'rule: jasms_2002_3_b1' do
      ...
      frags.map {|a| a.map(&:mass)}.flatten.inject(:+).should be_close(wt,MASS_DELTA)
    end
    specify 'rule: jasms_2002_3_b2' do
      ...
      frags.map {|a| a.map(&:mass)}.flatten.inject(:+).should be_close(wt-(Rubabel["[H]"].mass*2),MASS_DELTA)
    end
